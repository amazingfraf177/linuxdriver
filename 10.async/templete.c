...
static int templete_fasync (int fd,struct file * file, int on) {
        struct templete * save = (struct templete *) file->private_data;
        return fasync_helper(fd,file,on,&save->fasync);
}

static int templete_release(struct inode * inode,struct file * file){
    return templete_fasync(-1,file,0);
} 
...
static void timer_func(unsigned long arg) {
    ...
    if (atomic_read(&templete->keyrelease)){
        if(templete->fasync){
            kill_fasync(&templete->fasync,SIGIO,POLL_IN);
        }
    }
